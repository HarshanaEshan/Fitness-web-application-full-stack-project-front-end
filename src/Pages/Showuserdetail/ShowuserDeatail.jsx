...
function UserDetail() {
  ...
  const handleDelete = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/v1/user/deleteUser', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div>
      ...
      <Button  className="search-btn" onClick={handleDelete} sx={{ bgcolor: 'red', color: 'black', textTransform: 'none', width: { lg: '173px', xs: '80px' }, height: '56px', position: 'absolute', right: '0px', fontSize: { lg: '20px', xs: '14px' },position:'fixed',left:'5%' }} >Delete user</Button>
        </div>
      )}
